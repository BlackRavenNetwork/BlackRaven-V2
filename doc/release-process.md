...
* Update hardcoded [seeds](/contrib/seeds/README.md). TODO: Give example PR for BlackRaven
...
Build output expected:
  ...
  1. source tarball (`blackraven-${VERSION}.tar.gz`)
  2. linux 32-bit and 64-bit dist tarballs (`blackraven-${VERSION}-linux[32|64].tar.gz`)
  3. windows 32-bit and 64-bit unsigned installers and dist zips (`blackraven-${VERSION}-win[32|64]-setup-unsigned.exe`, `blackraven-${VERSION}-win[32|64].zip`)
  4. OS X unsigned installer and dist tarball (`blackraven-${VERSION}-osx-unsigned.dmg`, `blackraven-${VERSION}-osx64.tar.gz`)
...
Codesigner only: Sign the osx binary:
    ...
    transfer blackraven-osx-unsigned.tar.gz to osx for signing
    tar xf blackraven-osx-unsigned.tar.gz
...
Codesigner only: Sign the windows binaries:
    ...
    tar xf blackraven-win-unsigned.tar.gz
...
- Detached signatures will then be committed to the [blackraven-detached-sigs](https://github.com/The-BlackRaven-Endeavor/blackraven-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
```
blackraven-${VERSION}-aarch64-linux-gnu.tar.gz
blackraven-${VERSION}-arm-linux-gnueabihf.tar.gz
blackraven-${VERSION}-i686-pc-linux-gnu.tar.gz
blackraven-${VERSION}-x86_64-linux-gnu.tar.gz
blackraven-${VERSION}-osx64.tar.gz
blackraven-${VERSION}-osx.dmg
blackraven-${VERSION}.tar.gz
blackraven-${VERSION}-win32-setup.exe
blackraven-${VERSION}-win32.zip
blackraven-${VERSION}-win64-setup.exe
blackraven-${VERSION}-win64.zip
```
The `*-debug*` files generated by the Gitian build contain debug symbols
for troubleshooting by developers. It is assumed that anyone that is interested
in debugging can run Gitian to generate the files for themselves. To avoid
end-user confusion about which file to pick, as well as save storage
space *do not upload these to the blackraven.network server*.
