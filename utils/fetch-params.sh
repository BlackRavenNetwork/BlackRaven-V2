...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/BlackRavenParams"
else
    PARAMS_DIR="$HOME/.blackraven-params"
fi
...
DOWNLOAD_URL="https://github.com/BlackRavenNetwork/BlackRaven-V2/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with BLKR_DISABLE_SOMETHING=1
BLKR_DISABLE_WGET="${BLKR_DISABLE_WGET:-}"
#BLKR_DISABLE_IPFS="${BLKR_DISABLE_IPFS:-}"
BLKR_DISABLE_CURL="${BLKR_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$BLKR_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$BLKR_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$BLKR_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the BlackRaven parameters!
...
main() {
    ...
    cat <<EOF
BLKR - fetch-params.sh

This script will fetch the BlackRaven SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BlackRaven SNARK parameters. Note that it is
