...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for BLKR
    ...
    if (p_rewardSnapshot.strDistributionAsset != "BLKR") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is BLKR with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using BLKR differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "BLKR") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse BlackRaven address (already validated during ownership snapshot creation)
