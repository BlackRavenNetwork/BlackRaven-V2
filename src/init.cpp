...
#include "config/blackraven-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("blackraven-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all BlackRaven specific functionality (Smartnodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<blackravenaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/BlackRavenNetwork/BlackRaven-V2>";
    const std::string URL_WEBSITE = "<https://blackraven.network>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("blackraven-loadblk");
...
/** Sanity checks
 *  Ensure that BlackRaven is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BlackRaven version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single BlackRaven process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
	...
	GenerateBlackRavens(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
    ...
    // lite mode disables all BlackRaven-specific functionality
    fLiteMode = gArgs.GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, most BlackRaven-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/BlackRavenNetwork/BlackRaven-V2/pull/1817 and https://github.com/BlackRavenNetwork/BlackRaven-V2/pull/1743
    ...
    if (!Params().GetConsensus().fSmartnodesEnabled && gArgs.IsArgSet("-smartnodeblsprivkey")) {
        return InitError(_("Smartnodes are disabled on this network."));
    }
    ...
    // ********************************************************* Step 10c: schedule BlackRaven-specific tasks
