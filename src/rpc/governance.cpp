...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 BLACKRAVEN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on blackravencentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in blackraven.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("blackraven.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in blackraven.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* BlackRaven features */
    { "blackraven",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "blackraven",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "blackraven",               "gobject",                &gobject,                true,  {} },
    { "blackraven",               "voteraw",                &voteraw,                true,  {} },
