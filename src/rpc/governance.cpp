...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // users ignore all instructions on neoxacentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in neoxa.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("neoxa.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in neoxa.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* BlackRaven features */
    { "neoxa",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "neoxa",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "neoxa",               "gobject",                &gobject,                true,  {} },
    { "neoxa",               "voteraw",                &voteraw,                true,  {} },
