...
const char * const BITCOIN_CONF_FILENAME = "blackraven.conf";
const char * const BITCOIN_PID_FILENAME = "blackravend.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BlackRaven
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BlackRaven
    // Mac: ~/Library/Application Support/BlackRaven
    // Unix: ~/.blackraven
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BlackRaven";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BlackRaven";
#else
    // Unix
    return pathRet / ".blackraven";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty blackraven.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override blackraven.conf
