...
# blackraven_hash
RUN git clone https://github.com/The-BlackRaven-Endeavor/blackraven_hash
RUN cd blackraven_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} blackraven
RUN useradd -u ${USER_ID} -g blackraven -s /bin/bash -m -d /blackraven blackraven
...
RUN mkdir /blackraven-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /blackraven-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /blackraven-src
