...
// which are part of the BlackRaven repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="blackraven-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /blackraven-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "blackraven-blackraven/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from blackraven-blackraven/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'blackraven-blackraven/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from blackraven-blackraven/develop"
            }
          }

          if (hasCache) {
            sh "cd /blackraven-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /blackraven-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /blackraven-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /blackraven-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /blackraven-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /blackraven-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /blackraven-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
